        ...
        for el in self.game_map[-1]:
            if el == 1:
                if self.free_coins:
                    ...
                    Coin(self.speed, (self.screen_size[1] // 15, self.screen_size[1] // 15),
                ...
                if self.free_rocks:
                    ...
                    Rock(self.speed, (self.screen_size[1] // 9, self.screen_size[1] // 9),
                         (self.screen_size[0] + self.cell_size,
                          self.screen_size[1] - self.screen_size[1] // 5 + 5),
                ...
                if self.free_ghosts:
                    ...
                    Ghost(self.speed, (self.screen_size[1] // 9, self.screen_size[1] // 9),
                          (self.screen_size[0] + self.cell_size,
                           self.screen_size[1] - self.screen_size[1] // 5 - self.screen_size[1] // 8),
...
class Ghost(pg.sprite.Sprite):
    ...
    def __init__(self, speed, size, pos=(0, 0), *groups):
        ...
        self.images = [
            pg.transform.scale(
                load_im(f'ghost/fly_{i}.png').convert_alpha(), size
            )
            for i in range(self.anim_count)
        ]
        self.mask = pg.mask.from_surface(self.images[0])
    ...
    def draw(self, surf):
        surf.blit(self.images[round(self.anim_ind)], self.rect)
...
class Rock(pg.sprite.Sprite):
    def __init__(self, speed, size, pos=(0, 0), *groups):
        super().__init__(*groups)
        self.speed = speed
        self.image = pg.transform.scale(load_im('rock.png').convert_alpha(), size)
...
class Coin(pg.sprite.Sprite):
    def __init__(self, speed, size, pos=(0, 0), *groups):
        super().__init__(*groups)
        self.speed = speed
        self.image = pg.transform.scale(load_im('coin.png').convert_alpha(), size)
        self.mask = pg.mask.from_surface(self.image)
        self.rect = self.image.get_rect()
        self.rect.x, self.rect.y = pos[0], pos[1] - self.rect.size[1]
